...
- 文本 chunking 用 **Chonkie**（`processors/document/`），代码用 **tree-sitter**（`processors/code/`，或 Chonkie `CodeChunker`）。第三方库版本在 `pyproject.toml` pin 死、纳入 `chunker_version` fingerprint（见 04 §5.2 / 06 §6），升级走显式 bump + 重建，避免切分行为静默漂移。
